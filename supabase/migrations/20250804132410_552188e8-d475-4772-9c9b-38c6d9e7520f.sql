-- PHASE 2 & 3 : Configuration avancée et fonctions automatiques
...
-- Fonction pour calculer automatiquement la note moyenne et le nombre d'avis
...
-- Triggers pour mettre à jour automatiquement les notes
CREATE TRIGGER update_supplier_rating_on_review_insert
  AFTER INSERT ON public.suppliers_directory_reviews
  FOR EACH ROW EXECUTE FUNCTION public.update_supplier_rating();

CREATE TRIGGER update_supplier_rating_on_review_update
  AFTER UPDATE ON public.suppliers_directory_reviews
  FOR EACH ROW EXECUTE FUNCTION public.update_supplier_rating();

CREATE TRIGGER update_supplier_rating_on_review_delete
  AFTER DELETE ON public.suppliers_directory_reviews
  FOR EACH ROW EXECUTE FUNCTION public.update_supplier_rating();
...
SELECT 
  ...
  ARRAY['Livraison rapide', 'Qualité des pièces', 'Service client'],
  ARRAY['Prix un peu élevé'],
  true,
  'published'
