...
-- This migration addresses the security vulnerabilities by improving authorization checks
...
-- 2. Replace get_admin_subscription_overview function with regular function
...
BEGIN
    ...
    -- Add to user_roles table if it exists
    ...
    -- Log the action if security_audit_log table exists
...
EXCEPTION WHEN OTHERS THEN
    -- Log the failed attempt if security_audit_log table exists
...
COMMENT ON FUNCTION public.get_admin_subscription_overview IS 'Function with admin-only access control built into the query';