...
// Données statiques des départements français
const FRENCH_DEPARTMENTS: FrenchDepartment[] = [
...
export const useFrenchDepartments = () => {
  const [departments] = useState<FrenchDepartment[]>(FRENCH_DEPARTMENTS);
  const [regions] = useState<string[]>(() => 
    [...new Set(FRENCH_DEPARTMENTS.map(d => d.region))].sort()
  );
  const [regionMap] = useState<Map<string, FrenchDepartment[]>>(() => {
    const map = new Map<string, FrenchDepartment[]>();
    FRENCH_DEPARTMENTS.forEach(dep => {
      const existing = map.get(dep.region) || [];
      map.set(dep.region, [...existing, dep]);
    });
    return map;
  });
  const [loading] = useState(false);

  const getDepartmentsByRegion = (region: string): FrenchDepartment[] => {
  ...
  const getRegionByDepartment = (code: string): string | null => {
    const dep = departments.find(d => d.code === code);
