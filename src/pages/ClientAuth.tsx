...
const ClientAuth = () => {
  ...
  const { pendingAction } = usePendingQuoteAction();

  useEffect(() => {
    if (user && !loading) {
      // Si l'utilisateur est connecté et qu'il y a une action en attente
      if (pendingAction?.type === 'quote_request') {
        // Rediriger vers la page d'accueil où le modal de devis sera automatiquement restauré
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          {pendingAction?.type === 'quote_request' && (
            <p className="mt-2 text-center text-sm text-blue-600 font-medium">
              Votre demande de devis sera automatiquement restaurée après connexion
            </p>
          )}
        </div>
        <ClientAuthForm />
