...
// Widget types pour Dashboard
const dashboardWidgetTypes = [
  { id: 'overview-stats', name: 'Vue d\'ensemble', category: 'Analytics', type: 'overview' },
  { id: 'orders-list', name: 'Liste des commandes', category: 'Orders', type: 'orders' },
  { id: 'calendar-view', name: 'Calendrier', category: 'Schedule', type: 'calendar' },
  { id: 'inventory-status', name: 'État du stock', category: 'Inventory', type: 'inventory' },
  { id: 'revenue-chart', name: 'Graphique revenus', category: 'Analytics', type: 'revenue' },
  { id: 'alerts-panel', name: 'Alertes', category: 'Management', type: 'alerts' },
  { id: 'customer-metrics', name: 'Métriques clients', category: 'Customer', type: 'customers' },
  { id: 'performance-kpi', name: 'KPI Performance', category: 'Analytics', type: 'kpi' }
];
...
export const EnhancedDashboardTester: React.FC = () => {
  const [config, setConfig] = useState<DashboardConfiguration>(defaultConfig);
  const [deviceView, setDeviceView] = useState<'desktop' | 'tablet' | 'mobile'>('desktop');
  const [isPreviewMode, setIsPreviewMode] = useState(false);
  const [configName, setConfigName] = useState('');
  const [undoHistory, setUndoHistory] = useState<DashboardConfiguration[]>([]);
  const [redoHistory, setRedoHistory] = useState<DashboardConfiguration[]>([]);

  // Initialize sortable widgets
  const initialWidgets: SortableWidgetType[] = dashboardWidgetTypes.map((widget, index) => ({
    id: widget.id,
    type: widget.type,
    name: widget.name,
  ...
  const updateConfig = useCallback((updates: Partial<DashboardConfiguration>) => {
  ...
  const handleSaveConfiguration = async () => {
    ...
    await saveConfiguration({
      ...
      is_active: true,
      description: `Configuration Dashboard - ${new Date().toLocaleDateString()}`,
      tags: ['dashboard', 'interface', 'repairer']
    });
    
    toast({ title: "Succès", description: "Configuration Dashboard sauvegardée avec succès" });
    setConfigName('');
  };
      ...
      case 'overview':
        return (
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-3 rounded-lg border text-center">
            ...
            <p className="text-xs text-blue-600 mt-1">Statistiques générales</p>
          </div>
        );
      case 'orders':
        return (
          <div className="bg-gradient-to-br from-green-50 to-green-100 p-3 rounded-lg border text-center">
            ...
            <p className="text-xs text-green-600 mt-1">Gestion commandes</p>
          </div>
        );
      case 'calendar':
        return (
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-3 rounded-lg border text-center">
            ...
            <p className="text-xs text-purple-600 mt-1">Planning rendez-vous</p>
          </div>
        );
      case 'inventory':
        return (
          <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-3 rounded-lg border text-center">
            <Package className="h-6 w-6 mx-auto text-orange-600 mb-2" />
            <h4 className="font-semibold text-orange-800">{widget.name}</h4>
            <p className="text-xs text-orange-600 mt-1">Gestion stock</p>
          </div>
        );
      case 'revenue':
        return (
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 p-3 rounded-lg border text-center">
            ...
            <p className="text-xs text-emerald-600 mt-1">Analyse revenus</p>
          </div>
        );
      case 'alerts':
        return (
          <div className="bg-gradient-to-br from-red-50 to-red-100 p-3 rounded-lg border text-center">
            ...
            <p className="text-xs text-red-600 mt-1">Alertes système</p>
          </div>
        );
      default:
        return (
      ...
      <div className="border-b bg-card px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <h1 className="text-lg font-semibold">Test Interface Dashboard</h1>
          ...
          <div className={`p-6 transition-all duration-300 ${getDeviceClasses()}`}>
            <div style={{
              ...
              <RepairerDashboardTabs 
                ...
                appointments={[]}
                inventory={[]}
                profileData={{ name: 'Demo', rating: 4.8, totalRepairs: 100, joinDate: '2023-01-01' }}
                avgRepairTime={0}
              />
            </div>
          </div>
                    ...
                    <TabsContent value="layout" className="space-y-4">
                      <div>
                        ...
                        <div className="flex gap-2 mt-2">
                          {(['grid', 'masonry', 'compact'] as const).map((layout) => (
                            <Button
                              key={layout}
                              size="sm"
                    ...
                    <TabsContent value="save" className="space-y-4">
                      <div>
                        ...
                        <Input
                          ...
                          placeholder="Ma config Dashboard"
                        />
                      </div>
