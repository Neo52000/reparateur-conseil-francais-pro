...
import RepairerEcommerceInterface from '@/components/ecommerce/RepairerEcommerceInterface';

interface EcommerceConfiguration {
...
// Widget types pour E-commerce
const ecommerceWidgetTypes = [
  { id: 'product-grid', name: 'Grille produits', category: 'Products', type: 'product-grid' },
  { id: 'featured-products', name: 'Produits vedettes', category: 'Products', type: 'featured' },
  { id: 'product-categories', name: 'Catégories', category: 'Navigation', type: 'categories' },
  { id: 'shopping-cart', name: 'Panier', category: 'Cart', type: 'cart' },
  { id: 'product-reviews', name: 'Avis produits', category: 'Social', type: 'reviews' },
  { id: 'promotions-banner', name: 'Bannière promos', category: 'Marketing', type: 'promotions' },
  { id: 'customer-accounts', name: 'Comptes clients', category: 'Customer', type: 'accounts' },
  { id: 'order-tracking', name: 'Suivi commandes', category: 'Orders', type: 'tracking' },
  { id: 'payment-methods', name: 'Moyens de paiement', category: 'Payment', type: 'payment' },
  { id: 'shipping-options', name: 'Options livraison', category: 'Shipping', type: 'shipping' }
];

const defaultConfig: EcommerceConfiguration = {
  layout: 'grid',
  theme: 'light',
  customColors: {
...
export const EnhancedEcommerceTester: React.FC = () => {
  const [config, setConfig] = useState<EcommerceConfiguration>(defaultConfig);
  const [deviceView, setDeviceView] = useState<'desktop' | 'tablet' | 'mobile'>('desktop');
  const [isPreviewMode, setIsPreviewMode] = useState(false);
  const [configName, setConfigName] = useState('');
  const [undoHistory, setUndoHistory] = useState<EcommerceConfiguration[]>([]);
  const [redoHistory, setRedoHistory] = useState<EcommerceConfiguration[]>([]);

  // Initialize sortable widgets
  const initialWidgets: SortableWidgetType[] = ecommerceWidgetTypes.map((widget, index) => ({
    id: widget.id,
    type: widget.type,
    name: widget.name,
  ...
  const updateConfig = useCallback((updates: Partial<EcommerceConfiguration>) => {
  ...
  const handleSaveConfiguration = async () => {
    ...
    await saveConfiguration({
      ...
      is_active: true,
      description: `Configuration E-commerce - ${new Date().toLocaleDateString()}`,
      tags: ['ecommerce', 'interface', 'boutique']
    });
    
    toast({ title: "Succès", description: "Configuration E-commerce sauvegardée avec succès" });
    setConfigName('');
  };
      ...
      case 'product-grid':
        return (
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-3 rounded-lg border text-center">
            ...
            <p className="text-xs text-blue-600 mt-1">Affichage produits</p>
          </div>
        );
      case 'featured':
        return (
          <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 p-3 rounded-lg border text-center">
            <Star className="h-6 w-6 mx-auto text-yellow-600 mb-2" />
            <h4 className="font-semibold text-yellow-800">{widget.name}</h4>
            <p className="text-xs text-yellow-600 mt-1">Sélection spéciale</p>
          </div>
        );
      case 'categories':
        return (
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-3 rounded-lg border text-center">
            ...
            <p className="text-xs text-purple-600 mt-1">Navigation catégories</p>
          </div>
        );
      case 'cart':
        return (
          <div className="bg-gradient-to-br from-green-50 to-green-100 p-3 rounded-lg border text-center">
            ...
            <p className="text-xs text-green-600 mt-1">Gestion panier</p>
          </div>
        );
      case 'reviews':
        return (
          <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 p-3 rounded-lg border text-center">
            <Star className="h-6 w-6 mx-auto text-indigo-600 mb-2" />
            <h4 className="font-semibold text-indigo-800">{widget.name}</h4>
            <p className="text-xs text-indigo-600 mt-1">Avis clients</p>
          </div>
        );
      case 'promotions':
        return (
          <div className="bg-gradient-to-br from-red-50 to-red-100 p-3 rounded-lg border text-center">
            <Tag className="h-6 w-6 mx-auto text-red-600 mb-2" />
            <h4 className="font-semibold text-red-800">{widget.name}</h4>
            <p className="text-xs text-red-600 mt-1">Offres spéciales</p>
          </div>
        );
      case 'payment':
        return (
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 p-3 rounded-lg border text-center">
            <CreditCard className="h-6 w-6 mx-auto text-emerald-600 mb-2" />
            <h4 className="font-semibold text-emerald-800">{widget.name}</h4>
            <p className="text-xs text-emerald-600 mt-1">Paiements sécurisés</p>
          </div>
        );
      case 'shipping':
        return (
          <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-3 rounded-lg border text-center">
            <Truck className="h-6 w-6 mx-auto text-orange-600 mb-2" />
            <h4 className="font-semibold text-orange-800">{widget.name}</h4>
            <p className="text-xs text-orange-600 mt-1">Options livraison</p>
          </div>
        );
      default:
        return (
      ...
      <div className="border-b bg-card px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <h1 className="text-lg font-semibold">Test Interface E-commerce</h1>
            ...
            <div className="p-4 space-y-4">
              <div>
                ...
                <div className="space-y-2">
                  {ecommerceWidgetTypes.map((widget) => (
                    <Card 
                      key={widget.id}
                    ...
                    <TabsContent value="save" className="space-y-4">
                      <div>
                        ...
                        <Input
                          ...
                          placeholder="Ma config E-commerce"
                        />
                      </div>
