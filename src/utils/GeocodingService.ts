...
export class GeocodingService {
  // Utilisation de Nominatim (gratuit, pas de clé API requise)
  ...
  static async geocodeAddress(address: string, city?: string, postalCode?: string): Promise<GeocodingResult | null> {
    try {
      ...
      return {
        lat: Math.round(lat * 1000000) / 1000000, // 6 décimales de précision
  ...
  static async batchGeocode(addresses: Array<{address: string, city?: string, postal_code?: string}>): Promise<Array<GeocodingResult | null>> {
    const results: Array<GeocodingResult | null> = [];
    
    // Traitement séquentiel pour respecter les limites de l'API
    for (const addr of addresses) {
      const result = await this.geocodeAddress(addr.address, addr.city, addr.postal_code);
      results.push(result);
      
      // Délai entre les requêtes pour respecter les limites de Nominatim
  ...
  // Fallback vers les coordonnées départementales si le géocodage échoue
  static getFallbackCoordinates(postalCode: string): { lat: number; lng: number } {
    const departmentCode = postalCode.substring(0, 2);
    
    // Coordonnées centrales par département (principales villes)
    ...
    return departmentCoords[departmentCode] || { lat: 46.603354, lng: 1.888334 }; // Centre de la France
  }
}
